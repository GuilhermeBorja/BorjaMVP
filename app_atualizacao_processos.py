...
def atualizar_processo():
    ...
    if processos:
        ...
        if processo_selecionado:
            ...
            cursor.execute("SELECT * FROM etapas WHERE processo_id=?", (processo_id,))
            ...
            novo_data_term_ideal = st.text_input("Data de Término Ideal", value=processo["data_termino_ideal"], key="upd_data_term_ideal")
            novo_data_term_real = st.text_input("Data de Término Real", value=processo["data_termino_real"] if processo["data_termino_real"] else "", key="upd_data_term_real")
            
            st.subheader("Atualize os Dados Não Editáveis")
            st.text_input("Data de Criação", value=processo["data_criacao"], disabled=True)
            st.text_input("Tempo Total", value=str(processo["tempo_total"]), disabled=True)
            st.text_input("Status", value=processo["status"], disabled=True)
            ...
            for etapa in etapas:
                ...
                with col3:
                    novo_data_term_etapa = st.text_input(f"Término Real Etapa {etapa['id']}", value=etapa["data_termino_real"] if etapa["data_termino_real"] else "", key=f"upd_data_term_etapa_{etapa['id']}")
                novos_dados_etapas.append((etapa["id"], novo_nome_etapa, novo_resp_etapa, novo_data_term_etapa))
            
            if st.button("Atualizar Processo", key="btn_atualizar_processo"):
                conn = get_connection()
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE processos SET nome_processo=?, responsavel_geral=?, data_termino_ideal=?, data_termino_real=? WHERE id=?",
                    (novo_nome, novo_resp_geral, novo_data_term_ideal, novo_data_term_real, processo_id)
                )
